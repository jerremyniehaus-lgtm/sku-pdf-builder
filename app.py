...
# =========================
BASE_IMG_URL = "https://www.jared.com/productimages/processed/V-{SKU}_0_800.jpg?pristine=true"
BASE_PRODUCT_URL = "https://www.jared.com/p/V-{SKU}"
...
def sku_to_image_url(sku_numeric):
    return BASE_IMG_URL.replace("{SKU}", sku_numeric)

def sku_to_product_url(sku_numeric):
    return BASE_PRODUCT_URL.replace("{SKU}", sku_numeric)
...
def build_items_from_df(df):
    ...
    for _, row in df.iterrows():
        ...
        items_all.append(
            {
                "sku_label": sku_label,
                "img_url": sku_to_image_url(sku_num),
                "product_url": sku_to_product_url(sku_num),
...
if uploaded_file is not None:
    try:
        ...
        items_all = build_items_from_df(df)
        ...
        if st.button("Generate PDF"):
            ...
            file_name = f"SKU_Clickable_Grid_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            st.success("Done!")
